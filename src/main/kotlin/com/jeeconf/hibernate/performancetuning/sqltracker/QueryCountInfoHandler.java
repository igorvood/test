...
public class QueryCountInfoHandler implements QueryHandler {
    ...
    public void handleSql(String sql) {
        ...
        switch (queryType) {
            case SELECT:
                queryCountInfo.incrementSelectCount(sql);
                break;
            case INSERT:
                queryCountInfo.incrementInsertCount(sql);
                break;
            case UPDATE:
                queryCountInfo.incrementUpdateCount(sql);
                break;
            case DELETE:
                queryCountInfo.incrementDeleteCount(sql);
                break;
            case CALL:
                queryCountInfo.incrementCallCount(sql);
                break;
            default:
